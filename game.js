...
function respawnOrGameOver(p){
  if(G.lives<=0){ p.out=true;                  // 无共享命数 -> 出局
    if(!anyActive()){ recordScore(G.score); G.state='gameover'; stopMusic(); } return; }
  G.lives--;
...
function startGame(){ audio(); G.score=0; G.lives=DIFF().life*G.players + (G.players-1); G.state='play'; loadLevel(0); }
...
function drawTitle(){ resetT();
  ...
  X.fillStyle='#ffe680';X.font='bold 17px "Microsoft YaHei"';X.fillText('◀ 难度：'+DIFFS[SETTINGS.difficulty].name+' ▶   ( ← → 切换 )',VW/2,378);
  X.fillStyle=G.players===2?'#ffcf3a':'#9fd0ff';X.font='bold 17px "Microsoft YaHei"';
  X.fillText((G.players===2?'👥 双人合作':'👤 单人')+'   ( Shift 切换人数 )',VW/2,406);
  if(SAVE.best>0){ X.fillStyle='#cfe';X.font='13px "Microsoft YaHei"';X.fillText('🏆 最高分 '+SAVE.best,VW/2,430); }
  if(Math.floor(G.t/30)%2){ X.fillStyle='#fff';X.font='bold 23px "Microsoft YaHei"';X.fillText('按 Enter 或 点击屏幕 开始',VW/2,460); }
  X.textAlign='left';
}
...
function uiTick(){ G.t++;
  if(G.state==='title'){
    ...
    if(tapped('KeyP','ShiftLeft','ShiftRight')){ G.players=G.players===1?2:1; SFX.ui(); }   // 切换 1P/2P
    if(tapped('Enter','NumpadEnter'))startGame();
  }
  else if(G.state==='play'&&G.paused)updateMenu();
  else if(G.state==='gameover'){ if(tapped('Enter','NumpadEnter')){ G.score=0; G.lives=DIFF().life*G.players+(G.players-1); G.state='play'; loadLevel(G.levelIndex); } }
