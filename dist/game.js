...
/* ---------------- 输入方案 (单人 / 双人 / 手柄) ----------------
   双人键盘分区: P1=WASD+Space跳+左Shift射 ; P2=方向键+/跳+.射
   每名玩家也可用对应序号的手柄(GP0=P1, GP1=P2) */
const SCHEMES={
  solo:{l:['ArrowLeft','KeyA','GP0_l'], r:['ArrowRight','KeyD','GP0_r'], u:['ArrowUp','KeyW','GP0_u'], d:['ArrowDown','KeyS','GP0_d'], j:['Space','GP0_j'], f:['Fire','KeyJ','GP0_f'], gp:0},
  p1:{l:['KeyA','GP0_l'], r:['KeyD','GP0_r'], u:['KeyW','GP0_u'], d:['KeyS','GP0_d'], j:['Space','GP0_j'], f:['ShiftLeft','KeyF','GP0_f'], gp:0},
  p2:{l:['ArrowLeft','GP1_l'], r:['ArrowRight','GP1_r'], u:['ArrowUp','GP1_u'], d:['ArrowDown','GP1_d'], j:['Slash','GP1_j'], f:['Period','ShiftRight','GP1_f'], gp:1},
};
...
function pollGamepad(){
  ...
  const pads=navigator.getGamepads(); const now={}; let first=null;
  for(let i=0;i<Math.min(pads.length,2);i++){ const gp=pads[i]; if(!gp)continue; gpConnected=true; if(first==null)first=gp;
    const ax=gp.axes||[], bt=gp.buttons||[], bd=k=>bt[k]&&bt[k].pressed, pre='GP'+i+'_';
    if((ax[0]||0)<-0.4||bd(14))now[pre+'l']=1;
    if((ax[0]||0)>0.4||bd(15))now[pre+'r']=1;
    if((ax[1]||0)<-0.4||bd(12))now[pre+'u']=1;
    if((ax[1]||0)>0.4||bd(13))now[pre+'d']=1;
    if(bd(0))now[pre+'j']=1;                       // A 跳
    if(bd(2)||bd(7)||bd(5))now[pre+'f']=1;         // X/RT/RB 射击
    if(bd(9))now[i===0?'KeyP':'GP1_p']=1;          // Start 暂停
  }
  // 菜单/标题: 任意手柄方向+确认映射到通用键
  if(first){ const ax=first.axes||[], bt=first.buttons||[], bd=k=>bt[k]&&bt[k].pressed;
...
// 按方案查询(供各玩家用)
function pdown(sc,a){ return sc[a].some(k=>keys[k]); }
function ptapped(sc,a){ return sc[a].some(k=>press[k]); }

/* ---------------- 存档 & 设置 (版本化 v3: 加元进程统计) ---------------- */
const STORE='mc-contra.v3';
const SET_DEF={volume:0.7,muted:false,shake:true,difficulty:1,colorblind:false,reduceMotion:false,startWeapon:'normal',skin:0};
const SETTINGS=Object.assign({},SET_DEF);
const SAVE={ver:3,best:0,kills:0,clears:0,runs:0};   // 累计统计驱动解锁
...
function loadSave(){ try{
    ...
    if(!d){ const old=localStorage.getItem('mc-contra.v2')||localStorage.getItem('mc-contra.v1'); // 迁移旧档
      if(old)d=JSON.parse(old); }
    if(d){ if(typeof d.best==='number')SAVE.best=d.best|0;
      SAVE.kills=d.kills|0; SAVE.clears=d.clears|0; SAVE.runs=d.runs|0;
  ...
  SETTINGS.skin=clamp(SETTINGS.skin|0,0,99);
}
function persist(){ try{ localStorage.setItem(STORE,JSON.stringify(
  {ver:3,best:SAVE.best,kills:SAVE.kills,clears:SAVE.clears,runs:SAVE.runs,settings:SETTINGS})); }catch(e){} }
...
/* ---------------- 元进程 / 解锁 (M4-C) ----------------
   累计统计(击杀/通关/最高分)驱动解锁: 角色皮肤 + 初始武器。
   解锁状态由统计推导(非另存集合), 自洽不会丢. */
const SKINS=[
  {id:0, name:'士兵·绿', body:'#2f7d4a', dark:'#256340', band:'#b33',    cond:null,                 req:'初始'},
  {id:1, name:'游侠·橙', body:'#b5602a', dark:'#8f4a1f', band:'#ffd24a', cond:s=>s.clears>=1,        req:'通关 1 次'},
  {id:2, name:'暗影·紫', body:'#5a3a8c', dark:'#432c6b', band:'#c46bff', cond:s=>s.kills>=200,       req:'累计击杀 200'},
  {id:3, name:'黄金·金', body:'#c9a227', dark:'#9c7d18', band:'#fff2b0', cond:s=>s.best>=8000,       req:'最高分 8000'},
];
const START_WEAPONS=[
  {id:'normal',  name:'弓箭',   cond:null,            req:'初始'},
  {id:'machine', name:'机关枪', cond:s=>s.clears>=1,  req:'通关 1 次'},
  {id:'spread',  name:'喷射枪', cond:s=>s.kills>=120, req:'累计击杀 120'},
  {id:'flame',   name:'烈焰弹', cond:s=>s.kills>=320, req:'累计击杀 320'},
  {id:'laser',   name:'激光',   cond:s=>s.clears>=2,  req:'通关 2 次'},
];
function isUnlocked(cond){ return !cond || !!cond(SAVE); }
function skinUnlocked(i){ return isUnlocked(SKINS[clamp(i,0,SKINS.length-1)].cond); }
function weaponUnlocked(id){ const w=START_WEAPONS.find(w=>w.id===id); return !w||isUnlocked(w.cond); }
function unlockedSkinIdx(){ return SKINS.filter(s=>isUnlocked(s.cond)).map(s=>s.id); }
function unlockedWeaponIds(){ return START_WEAPONS.filter(w=>isUnlocked(w.cond)).map(w=>w.id); }
function cycleSkin(d){ const u=unlockedSkinIdx(); let i=u.indexOf(SETTINGS.skin|0); if(i<0)i=0;
  SETTINGS.skin=u[(i+d+u.length)%u.length]; persist(); SFX.ui(); }
function cycleWeapon(d){ const u=unlockedWeaponIds(); let i=u.indexOf(SETTINGS.startWeapon); if(i<0)i=0;
  SETTINGS.startWeapon=u[(i+d+u.length)%u.length]; persist(); SFX.ui(); }
function curSkin(){ const i=clamp(SETTINGS.skin|0,0,SKINS.length-1); return skinUnlocked(i)?SKINS[i]:SKINS[0]; }
function curStartWeapon(){ return weaponUnlocked(SETTINGS.startWeapon)?SETTINGS.startWeapon:'normal'; }
// 当前已解锁项的标识列表(用于检测"本局新解锁")
function unlockSnapshot(){ const a=[];
  for(const s of SKINS)if(s.cond&&isUnlocked(s.cond))a.push('皮肤·'+s.name);
  for(const w of START_WEAPONS)if(w.cond&&isUnlocked(w.cond))a.push('武器·'+w.name);
  return a; }
function endRun(cleared){
  if(G.runEnded)return; G.runEnded=true;
  const before=unlockSnapshot();
  SAVE.runs=(SAVE.runs|0)+1; SAVE.kills=(SAVE.kills|0)+(G.runKills|0);
  if(cleared)SAVE.clears=(SAVE.clears|0)+1;
  if(G.score>SAVE.best)SAVE.best=G.score;
  persist();
  const after=unlockSnapshot();
  G.newUnlocks=after.filter(x=>!before.includes(x));
}
// 把所选初始武器/皮肤应用到本局玩家(开新局时调用)
function applyLoadout(){
  const sw=curStartWeapon(); if(WEAPONS[sw])for(const p of players)p.weapon=sw;
  const sk=curSkin(); if(players[0])players[0].col={body:sk.body,dark:sk.dark,band:sk.band,name:'P1'};
}
...
function musicTick(){
  ...
  // 自适应三档: 探索(慢) → 战斗(热度>0.4, 中) → Boss(intense, 快+高八度琶音)
  const combat = !MUSIC.intense && G.heat>0.4;
  MUSIC.frame++; const period=MUSIC.intense?6:(combat?8:11);
  if(MUSIC.frame<period)return; MUSIC.frame=0;
  const s=MUSIC.step%16, dense=MUSIC.intense||combat;
  const b=BASS[s]; if(b!==undefined&&(s%2===0))blip(noteHz(b)/2,0.16,'triangle',0.16,0,musicGain);
  if(dense||s%2===0){ const ar=ARP[s]; if(ar!==undefined)blip(noteHz(ar+(MUSIC.intense?12:0)),0.09,'square',0.05,0,musicGain); }
  if(s%(dense?4:8)===0)noise(0.04,0.06,3000); // hat(战斗更密)
  MUSIC.step++;
}
...
const G={ state:'title', levelIndex:0, lvl:null, cam:{x:0,y:0}, camLead:0,
  ...
  t:0, score:0, combo:0, comboT:0, fade:0, players:1, lives:3,
  bossLock:false, bossLockX:0, message:'', msgT:0, paused:false, overlay:null, menuIndex:0,
  titleSel:0, galSel:0, runKills:0, runEnded:false, newUnlocks:[],   // M4-C 元进程
  heat:0 };   // M4-D 战斗热度(驱动自适应配乐)
let players=[], player, enemies, bullets, ebullets, powerups, particles, floaters, boss;
// 存活/索敌辅助
function alivePlayers(){ return players.filter(p=>!p.dead&&!p.out); }
function anyActive(){ return players.some(p=>!p.out); }            // 还有玩家未彻底出局
function nearestAlive(e){ const a=alivePlayers(); if(!a.length)return null;
  let best=a[0],bd=1e9; for(const p of a){ const d=Math.abs((p.x+p.w/2)-(e.x+e.w/2))+Math.abs((p.y+p.h/2)-(e.y+e.h/2)); if(d<bd){bd=d;best=p;} } return best; }
...
/* ============================================================
   玩家
   ============================================================ */
const PCOLORS=[{body:'#2f7d4a',dark:'#256340',band:'#b33',name:'P1'},{body:'#2f5a9c',dark:'#244a7d',band:'#ffcf3a',name:'P2'}];
function makePlayer(x,y,idx){
  idx=idx||0; const sc = G.players===2 ? (idx===0?SCHEMES.p1:SCHEMES.p2) : SCHEMES.solo;
  return {x,y,w:24,h:38, vx:0,vy:0, dir:1, onGround:false, prone:false,
    hp:4, maxHp:4, iframe:90, dead:false, deadT:0, out:false,
    weapon:'normal', fireCD:0, jumps:0, coyote:0, jumpBuf:0,
    aimX:1, aimY:0, walkAnim:0, muzzle:0, win:false,
    idx, scheme:sc, col:PCOLORS[idx]||PCOLORS[0] };
}
...
function updatePlayer(p){
  ...
  if(p.dead){ p.deadT--; p.vy+=0.6; moveEntity(p);
    if(p.deadT<=0)respawnOrGameOver(p); return; }
  if(p.win){ p.vy+=0.6; moveEntity(p); return; }

  const L=pdown(sc,'l'), R=pdown(sc,'r'), U=pdown(sc,'u'), D=pdown(sc,'d');
  let h=(R?1:0)-(L?1:0);
  if(h!==0)p.dir=h;
  ...
  // 跳跃(coyote + buffer)
  const jt=ptapped(sc,'j');
  if(jt)p.jumpBuf=7; if(p.jumpBuf>0)p.jumpBuf--; if(p.coyote>0)p.coyote--;
  if(p.jumpBuf>0 && (p.onGround||p.coyote>0) && !p.prone){ p.vy=-12; p.jumps=1; p.jumpBuf=0; p.coyote=0; SFX.jump(panFor(p.x)); }
  else if(jt && !p.onGround && p.coyote<=0 && p.jumps>=1 && p.jumps<2){ p.vy=-10.5; p.jumps=2; SFX.jump(panFor(p.x)); burst(p.x+p.w/2,p.y+p.h,'#fff',6,3); }
  if(pdown(sc,'j') && p.vy<0)p.vy-=0.3;
  ...
  // 开火(按住连发)
  if(pdown(sc,'f') && p.fireCD<=0)fireWeapon(p);

  if(touchingDeadly(p))hurtPlayer(p,99);
  if(p.y>G.lvl.pxH+80)hurtPlayer(p,99);

  // 拾取道具
  for(const u of powerups){ if(!u.got && aabb(p,u)){ u.got=true; pickup(p,u.kind); } }
...
function fireWeapon(p){
  const w=WEAPONS[p.weapon]; p.fireCD=w.cd; p.muzzle=7; G.heat=Math.min(1,G.heat+0.06);
...
function pickup(p,kind){
  ...
  floaters.push({kind:'text',x:p.x,y:p.y-10,vy:-0.7,life:50,
    text: kind==='health'?'+❤':WEAPONS[WEAPON_FROM_KIND[kind]].name, color:'#fff', big:true});
  burst(p.x+p.w/2,p.y+p.h/2,'#bdf',14,4);
  if(kind==='health'){ p.hp=Math.min(p.maxHp,p.hp+1); }
  else { p.weapon=WEAPON_FROM_KIND[kind]; }
}
function hurtPlayer(p,dmg){
  if(p.iframe>0||p.dead||p.win||p.out)return;
  G.heat=1;
  ...
  if(p.hp<=0){ killPlayer(p); } else { p.iframe=64; p.vy=-6; p.vx=-p.dir*4; }
}
function killPlayer(p){ if(p.dead)return;
...
function respawnOrGameOver(p){
  if(G.lives<=0){ p.out=true;                  // 无共享命数 -> 出局
    if(!anyActive()){ endRun(false); G.state='gameover'; stopMusic(); } return; }
  G.lives--;
  // 复活: 优先靠近存活队友, 否则镜头左侧安全地面
  const mate=alivePlayers()[0];
  const rx=clamp(mate?mate.x:(-G.cam.x+120), 0, G.lvl.pxW-40);
...
function updateBullets(){
  ...
  for(const b of ebullets){ if(b.grav)b.vy+=b.grav; b.x+=b.vx; b.y+=b.vy; b.life--;
    for(const p of players){ if(p.iframe<=0 && !p.dead && !p.out && aabb(b,p)){ hurtPlayer(p,1); b.life=0; break; } }
...
function killEnemy(e){ G.combo++; G.comboT=120; G.runKills=(G.runKills|0)+1;
...
function creeperBoom(e){ const pan=panFor(e.x+e.w/2); SFX.explode(pan); addShake(0.55); addFlash(8,'#ffb060'); G.hitstop=Math.max(G.hitstop,3);
  ...
  for(const p of players){ if(p.iframe<=0&&!p.dead&&!p.out && Math.hypot((e.x+e.w/2)-(p.x+p.w/2),(e.y+e.h/2)-(p.y+p.h/2))<70)hurtPlayer(p,1); } }
function updateEnemy(e){
  ...
  const tg=nearestAlive(e), hasT=!!tg;
  const pcx=tg?tg.x+tg.w/2:e.x+e.w/2, pcy=tg?tg.y+tg.h/2:e.y+e.h/2;
  const ex=e.x+e.w/2, ey=e.y+e.h/2, ddx=pcx-ex;

  if(e.type==='blaze'){ // 飞行 + 三连火球
    e.y=e.baseY + Math.sin(e.t*0.05)*40;
    e.x += (hasT&&Math.abs(ddx)>200? sign(ddx)*e.speed : Math.sin(e.t*0.02)*0.6);
    if(hasT&&Math.abs(ddx)<460){ e.shoot--; if(e.shoot<=0){ e.shoot=110*ef;
  ...
  if(e.type==='zombie'){
    if(hasT&&Math.abs(ddx)<320)e.dir=sign(ddx)||e.dir;
    ...
    if(hasT&&Math.abs(ddx)<260 && Math.abs(pcy-ey)<90){ e.dir=sign(ddx)||e.dir; e.vx=e.dir*e.speed; if(Math.abs(ddx)<42)e.armed=true; }
    ...
    e.vx=0; moveEntity(e);
    if(hasT&&Math.abs(ddx)<460){ e.dir=sign(ddx)||e.dir; e.shoot--; if(e.shoot<=0){ e.shoot=rand(70,110)*ef;
      const a=Math.atan2(pcy-ey,pcx-ex); efire(ex,ey-4,a,5,{color:'#dcdcdc',grav:0.02,w:14,h:6}); SFX.shoot(pan); } }
  } else if(e.type==='turret'){
    moveEntity(e);
    if(hasT&&Math.abs(ddx)<520){ e.shoot--; if(e.shoot<=0){ e.shoot=64*ef;
      const a=Math.atan2(pcy-ey,pcx-ex); for(let k=-1;k<=1;k++)efire(ex,ey,a+k*0.14,4.6,{color:'#ff7a3d'}); SFX.shoot2(pan); } }
  }
  // 接触伤害(任一玩家)
  if(!e.dead)for(const p of players){ if(p.iframe<=0&&!p.dead&&!p.out&&aabb(p,e)){ hurtPlayer(p,1); break; } }
}
...
function updateBoss(){
  ...
  const tg=nearestAlive(b)||players[0]||{x:b.x,y:b.y,w:0,h:0};
  const pcx=tg.x+tg.w/2, pcy=tg.y+tg.h/2;
  ...
  for(const p of players){ if(p.iframe<=0&&!p.dead&&!p.out&&aabb(p,b)){ hurtPlayer(p,1); break; } }
}
function makeAdd(x,y){ return {x,y,vx:0,vy:0,dir:-1,onGround:false,dead:false,hp:2,type:'zombie',
  t:0,anim:0,hitWall:false,flash:0,knock:0,speed:1.4,shoot:0,w:26,h:36}; }
...
function loadLevel(i){
  ...
  const sx=G.lvl.start.x, sy=G.lvl.start.y;
  players=[]; for(let k=0;k<G.players;k++)players.push(makePlayer(sx+k*40, sy, k));
  player=players[0];
...
function nextLevel(){
  ...
  else { endRun(true); G.state='clear'; stopMusic(); SFX.win(); } }
function startGame(){ audio(); G.score=0; G.runKills=0; G.runEnded=false; G.newUnlocks=[];
  G.lives=DIFF().life*G.players + (G.players-1); G.state='play'; loadLevel(0); applyLoadout(); }
// 失败后重试: 同样视为新一局(重置统计+应用装备)
function retryRun(){ G.score=0; G.runKills=0; G.runEnded=false; G.newUnlocks=[];
  G.lives=DIFF().life*G.players+(G.players-1); G.state='play'; loadLevel(G.levelIndex); applyLoadout(); }
...
function drawSoldier(p){
  ...
  const cx=p.x+p.w/2, f=p.dir, col=p.col||PCOLORS[0];
  X.translate(cx,p.y);
  const swing=p.onGround&&!p.prone?Math.sin(p.walkAnim)*6:3;
  if(p.prone){
    X.fillStyle=col.body; X.fillRect(-16,p.h-12,32,12);
    X.fillStyle='#caa07a'; X.fillRect(f>0?10:-22,p.h-18,12,8);
    X.strokeStyle='#39424f'; X.lineWidth=3; X.beginPath(); X.moveTo(0,p.h-14); X.lineTo(f*22,p.h-14); X.stroke();
  } else {
    // 程序化多帧姿态: 待机呼吸 / 跑步腿循环 / 跳跃收腿
    const moving=p.onGround&&Math.abs(p.vx)>0.4, air=!p.onGround;
    const by = moving? -Math.abs(Math.sin(p.walkAnim))*1.6 : (p.onGround? Math.sin(G.t*0.08)*1.0 : 0);
    X.translate(0,by);
    X.fillStyle='#39424f';
    if(air){                          // 跳跃: 前腿收, 后腿蹬
      X.save();X.translate(-5,22);X.rotate(-0.5);X.fillRect(-4,0,8,14);X.restore();
      X.save();X.translate(5,22);X.rotate(0.7);X.fillRect(-4,0,8,13);X.restore();
    } else if(moving){                // 跑: 两腿交替摆动
      X.save();X.translate(-5,22);X.rotate(swing*0.07);X.fillRect(-4,0,8,15);X.restore();
      X.save();X.translate(5,22);X.rotate(-swing*0.07);X.fillRect(-4,0,8,15);X.restore();
    } else { X.fillRect(-9,22,8,15); X.fillRect(1,22,8,15); }   // 待机
    X.fillStyle=col.body; X.fillRect(-10,6,20,18); X.fillStyle=col.dark; X.fillRect(-10,6,20,4);
    X.fillStyle='#caa07a'; X.fillRect(-9,-12,18,18);
    X.fillStyle=col.band; X.fillRect(-10,-13,20,6);
    ...
    X.fillStyle='#39424f'; X.fillRect(12,-4,16,8);
    if(p.muzzle>0){ X.fillStyle='#ffe066'; X.beginPath(); X.arc(30,0,6+p.muzzle,0,7); X.fill();
      X.fillStyle='rgba(255,240,150,.5)'; X.beginPath(); X.arc(30,0,12+p.muzzle,0,7); X.fill(); }   // 枪口辉光
    X.restore();
  }
  // 双人: 头顶标记 P1/P2
  if(G.players===2){ X.globalAlpha=1; X.fillStyle=col.band; X.font='bold 10px monospace'; X.textAlign='center';
    X.fillText(col.name, 0, p.prone?p.h-24:-20); X.textAlign='left'; }
  X.restore();
}
function drawEnemy(e){
  X.save(); X.translate(e.x+e.w/2,e.y);
  if(e.flash>0){ X.globalAlpha=0.9; const s=1+e.flash*0.012; X.scale(s,2-s); }   // 命中挤压反馈
  if(e.type==='zombie'){ const sw=Math.sin(e.anim)*5;
    ...
    const tg=nearestAlive(e)||players[0]||{x:e.x,y:e.y,w:0,h:0};
    const a=Math.atan2((tg.y+tg.h/2)-(e.y+e.h/2),(tg.x+tg.w/2)-(e.x+e.w/2));
...
function drawBoss(){ const b=boss; X.save(); X.translate(b.x+b.w/2,b.y+b.h/2);
  ...
  if(b.hp<b.maxHp*0.4 && !b.defeated && motionOK()){   // 残血狂暴: 红色脉动光环
    X.save(); X.globalAlpha=0.25+0.2*Math.sin(b.t*0.3); X.strokeStyle='#ff3b3b'; X.lineWidth=6;
    X.strokeRect(-w/2-7,-h/2-7,w+14,h+14); X.restore(); }
  if(b.type==='fortress'){
...
function drawBackground(){ const sky=G.lvl.def.sky, biome=G.lvl.def.biome;
  ...
  if(biome==='end'){ X.fillStyle='rgba(255,255,255,.6)';
    ...
    // 远山(慢视差) + 近山(快视差) = 多层纵深
    X.fillStyle='rgba(30,90,50,.35)';for(let i=0;i<6;i++){ const px=((i*240+G.cam.x*0.22)%(VW+260)+VW+260)%(VW+260)-130; X.beginPath();X.moveTo(px,VH);X.lineTo(px+120,VH-190);X.lineTo(px+240,VH);X.fill(); }
...
// 氛围粒子(纯程序化, 不分配对象): 丛林萤火虫 / 下界火星 / 末地虚空微尘
function drawAmbient(biome){ const t=G.t, par=-G.cam.x;
  if(biome==='nether'){
    for(let i=0;i<26;i++){ const x=((i*101-par*0.5+Math.sin((t+i*40)*0.02)*20)%(VW+40)+VW+40)%(VW+40)-20;
      const y=VH-((i*67+t*1.6)%(VH+40))+20; X.fillStyle='rgba(255,'+(120+(i%3)*45)+',40,.6)'; X.fillRect(x,y,2,3); }
  } else if(biome==='end'){
    X.fillStyle='rgba(200,120,255,.5)';
    for(let i=0;i<24;i++){ const x=((i*149-par*0.4)%(VW+40)+VW+40)%(VW+40)-20;
      const y=((i*97+Math.sin((t+i*30)*0.03)*30+t*0.4)%(VH+40)+VH+40)%(VH+40)-20; X.fillRect(x,y,2,2); }
  } else {
    for(let i=0;i<20;i++){ const a=0.35+0.4*Math.sin((t+i*55)*0.05);
      const x=((i*131-par*0.45+Math.sin((t+i*20)*0.03)*30)%(VW+40)+VW+40)%(VW+40)-20;
      const y=((i*83+Math.sin((t+i*47)*0.02)*40)%(VH-150))+90; X.fillStyle='rgba(190,255,120,'+a+')'; X.fillRect(x,y,2,2); }
  }
}
...
function drawPlayerHUD(p,x,y,alignRight){
  const col=p.col||PCOLORS[0];
  if(G.players===2){ X.fillStyle=col.band;X.font='bold 11px monospace';X.textAlign=alignRight?'right':'left';
    X.fillText(col.name, alignRight?x:x, y-4); }
  for(let i=0;i<p.maxHp;i++){ const hx=alignRight?(x-14-i*22):(x+i*22),hy=y;
    X.fillStyle='#3a0000';heart(hx,hy); if(!p.out&&p.hp>i){ X.fillStyle=col.band;heart(hx,hy); } }
  // 武器小图标
  const w=WEAPONS[p.weapon], wx=alignRight?x-2:x, wy=y+20;
  X.fillStyle=w.color;X.fillRect(alignRight?wx-16:wx,wy,14,14);
  X.fillStyle='#10202c';X.font='bold 11px monospace';X.textAlign='center';X.fillText(w.letter,alignRight?wx-9:wx+7,wy+11);
  X.textAlign='left';
}
function drawHUD(){
  drawPlayerHUD(players[0],14,18,false);
  if(G.players===2&&players[1])drawPlayerHUD(players[1],VW-14,18,true);
  // 共享命数(居中)
  X.fillStyle='rgba(0,0,0,.4)';X.fillRect(VW/2-44,8,88,24);
  X.fillStyle='#fff';X.font='bold 15px "Microsoft YaHei"';X.textAlign='center';
  X.fillText('余命 × '+Math.max(0,G.lives), VW/2, 25); X.textAlign='left';
  // 分数(中下偏右, 避免与 P2 HUD 冲突)
  X.fillStyle='rgba(0,0,0,.4)';X.fillRect(VW-200,VH-52,186,24);
  X.fillStyle='#bfe';X.font='14px monospace';X.fillText('分数 '+G.score,VW-194,VH-35);
  if(SAVE.best>0){ X.fillStyle='#ffe680';X.font='11px monospace';X.textAlign='right';X.fillText('最高 '+SAVE.best,VW-16,VH-35);X.textAlign='left'; }
  // 连击
...
/* ---------------- 主渲染 ---------------- */
function render(){
  // 摄像机: 跟随存活玩家中点(双人) + 朝向前瞻 + 平滑
  const focus=alivePlayers().length?alivePlayers():players;
  let mx=0,my=0; for(const p of focus){ mx+=p.x+p.w/2; my+=p.y+p.h/2; } mx/=focus.length; my/=focus.length;
  const lead=(focus.length===1?focus[0].dir:0)*120;
  G.camLead=lerp(G.camLead,lead,0.05);
  let tx=mx - VW/2 + G.camLead;
  let ty=my - VH/2 - 30;
  ...
  X.save();X.translate(ox,oy); for(const p of players)if(!p.out)drawSoldier(p); X.restore();
...
function drawTitle(){ resetT();
  ...
  for(let i=0;i<16;i++){ const x=(i*71+G.t*0.7)%(VW+60)-30,y=60+(i%5)*92+Math.sin(G.t*0.02+i)*18;
    const cols=['#5bbf4a','#7a5230','#7f7f7f','#2f7d4a','#ff7a3d'];X.fillStyle=cols[i%5];X.globalAlpha=.4;X.fillRect(x,y,30,30);X.globalAlpha=1; }
  X.textAlign='center';
  X.fillStyle='#0a0a0a';X.font='bold 60px "Microsoft YaHei"';X.fillText('MC 魂斗罗',VW/2+4,138);
  X.fillStyle='#ffcf3a';X.fillText('MC 魂斗罗',VW/2,134);
  X.fillStyle='#eaf6ff';X.font='17px "Microsoft YaHei"';X.fillText('—— 我的世界版魂斗罗 · 跑射闯关 ——',VW/2,170);
  X.fillStyle='#9fd0ff';X.font='12px "Microsoft YaHei"';
  X.fillText('← → 移动  空格 跳  ↑↓ 瞄准/卧倒  J/鼠标 射击(按住连发)  P 暂停',VW/2,202);
  // 光标菜单
  const tg=titleGeom();
  for(const r of tg.rows){ const sel=r.i===G.titleSel;
    X.fillStyle=sel?'rgba(95,208,255,.26)':'rgba(0,0,0,.34)';roundRect(r.x,r.y,r.w,r.h,9);X.fill();
    if(sel){X.strokeStyle='#5fd0ff';X.lineWidth=2;roundRect(r.x,r.y,r.w,r.h,9);X.stroke();}
    X.fillStyle=sel?'#fff':'#cfe';X.font='bold 16px "Microsoft YaHei"';X.textAlign='left';X.fillText(r.m.label,r.x+16,r.y+23);
    const adj=(r.m.id!=='start'&&r.m.id!=='gallery'); let val=null;
    if(r.m.id==='difficulty')val=DIFFS[SETTINGS.difficulty].name;
    else if(r.m.id==='players')val=G.players===2?'双人 2P':'单人 1P';
    else if(r.m.id==='weapon')val=(START_WEAPONS.find(w=>w.id===curStartWeapon())||START_WEAPONS[0]).name;
    else if(r.m.id==='skin')val=curSkin().name;
    X.textAlign='right';
    if(val!=null){ X.fillStyle='#ffe680';X.fillText((adj?'◀ ':'')+val+(adj?' ▶':''),r.x+r.w-14,r.y+23); }
    else if(r.m.id==='gallery'){ const u=unlockSnapshot().length, tot=SKINS.length-1+START_WEAPONS.length-1;
      X.fillStyle='#9fd0ff';X.fillText(u+'/'+tot+' 已解锁',r.x+r.w-14,r.y+23); }
    X.textAlign='left';
  }
  X.textAlign='center';
  if(SAVE.best>0||SAVE.kills>0){ X.fillStyle='#cfe';X.font='12px "Microsoft YaHei"';
    X.fillText('🏆 最高分 '+SAVE.best+'   累计击杀 '+SAVE.kills+'   通关 '+SAVE.clears,VW/2,506); }
  if(Math.floor(G.t/30)%2){ X.fillStyle='#fff';X.font='15px "Microsoft YaHei"';X.fillText('↑↓ 选择 · ←→ 调整 · Enter/点击 确认',VW/2,528); }
  X.textAlign='left';
}
function drawGallery(){ resetT();
  X.fillStyle='rgba(8,12,20,.97)';X.fillRect(0,0,VW,VH);
  X.textAlign='center';X.fillStyle='#ffcf3a';X.font='bold 32px "Microsoft YaHei"';X.fillText('🔓 解锁图鉴',VW/2,58);
  X.fillStyle='#cfe';X.font='13px "Microsoft YaHei"';
  X.fillText('累计击杀 '+SAVE.kills+' · 通关 '+SAVE.clears+' · 最高分 '+SAVE.best,VW/2,84);
  const colW=400,x1=VW/2-colW-12,x2=VW/2+12;
  X.textAlign='left';
  const col=(title,x,items,name)=>{
    X.fillStyle='#9fd0ff';X.font='bold 17px "Microsoft YaHei"';X.fillText(title,x,120);
    let y=136;
    for(const it of items){ const ok=isUnlocked(it.cond);
      X.fillStyle=ok?'rgba(60,200,120,.15)':'rgba(255,255,255,.05)';roundRect(x,y,colW,44,8);X.fill();
      X.strokeStyle=ok?'#3df58a':'#3a4452';X.lineWidth=1.5;roundRect(x,y,colW,44,8);X.stroke();
      X.fillStyle=ok?'#fff':'#7f8a99';X.font='bold 15px "Microsoft YaHei"';X.fillText((ok?'✓ ':'🔒 ')+name(it),x+14,y+20);
      X.fillStyle=ok?'#bfe':'#8895a6';X.font='12px "Microsoft YaHei"';X.fillText(ok?'已解锁':('解锁：'+it.req),x+14,y+37);
      y+=52;
    }
  };
  col('角色皮肤',x1,SKINS,s=>s.name);
  col('初始武器',x2,START_WEAPONS,w=>w.name);
  X.textAlign='center';X.fillStyle='#fff';X.font='15px "Microsoft YaHei"';
  if(Math.floor(G.t/30)%2)X.fillText('Enter / Esc / 点击 返回',VW/2,VH-22);
  X.textAlign='left';
}
function drawEnd(win){ resetT();
  ...
  X.fillText(rec?('🎉 新纪录！历史最高 '+SAVE.best):('历史最高 '+SAVE.best),VW/2,VH/2+38);
  // 本局新解锁
  if(G.newUnlocks&&G.newUnlocks.length){
    X.fillStyle='#7cff9a';X.font='bold 16px "Microsoft YaHei"';
    X.fillText('🔓 新解锁：'+G.newUnlocks.join(' · '),VW/2,VH/2+66);
  }
  X.fillStyle='#cfe';X.font='15px "Microsoft YaHei"';X.fillText(win?'按 Enter / 点击 回到标题':'按 Enter / 点击 重新开始本关',VW/2,VH/2+96);
  X.textAlign='left';
}
...
function stepPlay(){ G.t++;
  ...
  if(G.heat>0)G.heat=Math.max(0,G.heat-0.004);   // 战斗热度衰减(无交战渐回探索)
  if(G.msgT>0)G.msgT--;
}
function uiTick(){ G.t++;
  if(G.state==='title')updateTitle();
  else if(G.state==='gallery'){
    if(tapped('Enter','NumpadEnter','Escape','KeyP')){ G.state='title'; SFX.ui(); }
    if(pointer.justDown){ G.state='title'; SFX.ui(); }
  }
  else if(G.state==='play'&&G.paused)updateMenu();
  else if(G.state==='gameover'){ if(tapped('Enter','NumpadEnter'))retryRun(); }
...
const TITLE_ROWS=[
  ...
  {id:'players',label:'人数'},
  {id:'weapon',label:'初始武器'},
  {id:'skin',label:'角色皮肤'},
  {id:'gallery',label:'解锁图鉴'},
];
function titleGeom(){ const bw=380,rh=40,x=(VW-bw)/2,y0=250;
  return {x,y0,bw,rh,rows:TITLE_ROWS.map((m,i)=>({m,i,x,y:y0+i*rh,w:bw,h:rh-6}))}; }
function titleActivate(id,click){
  if(id==='gallery'){ G.state='gallery'; G.galSel=0; SFX.ui(); }
  else if(id==='difficulty'){ if(click){SETTINGS.difficulty=(SETTINGS.difficulty+1)%3;persist();SFX.ui();} else startGame(); }
  else if(id==='players'){ if(click){G.players=G.players===1?2:1;SFX.ui();} else startGame(); }
  else if(id==='weapon'){ if(click)cycleWeapon(1); else startGame(); }
  else if(id==='skin'){ if(click)cycleSkin(1); else startGame(); }
  else startGame();
}
function updateTitle(){
  const g=titleGeom();
  for(const r of g.rows)if(hit(r))G.titleSel=r.i;
  if(tapped('ArrowUp','KeyW')){ G.titleSel=(G.titleSel+TITLE_ROWS.length-1)%TITLE_ROWS.length; SFX.ui(); }
  if(tapped('ArrowDown','KeyS')){ G.titleSel=(G.titleSel+1)%TITLE_ROWS.length; SFX.ui(); }
  const cur=TITLE_ROWS[G.titleSel], L=tapped('ArrowLeft','KeyA'), R=tapped('ArrowRight','KeyD');
  if(cur.id==='difficulty'){ if(L){SETTINGS.difficulty=(SETTINGS.difficulty+2)%3;persist();SFX.ui();} if(R){SETTINGS.difficulty=(SETTINGS.difficulty+1)%3;persist();SFX.ui();} }
  else if(cur.id==='players'){ if(L||R){G.players=G.players===1?2:1;SFX.ui();} }
  else if(cur.id==='weapon'){ if(L)cycleWeapon(-1); if(R)cycleWeapon(1); }
  else if(cur.id==='skin'){ if(L)cycleSkin(-1); if(R)cycleSkin(1); }
  if(tapped('ShiftLeft','ShiftRight')){ G.players=G.players===1?2:1; SFX.ui(); }   // 快捷切人数
  if(pointer.justDown){ const row=g.rows.find(r=>hit(r));
    if(row){ G.titleSel=row.i; titleActivate(row.m.id,true); return; }
    else { startGame(); return; } }                       // 点空白处=开始
  if(tapped('Enter','NumpadEnter')) titleActivate(cur.id,false);
}
function draw(){
  if(G.state==='title')drawTitle();
  else if(G.state==='gallery')drawGallery();
...
function autoPause(){ if(G.state==='play'&&!G.paused&&players.length&&anyActive())pauseGame(); }
...
if(typeof location!=='undefined'&&/(?:^|[?&])debug\b/.test(location.search)){
  window.__c={ get G(){return G;}, get player(){return players[0];}, get players(){return players;}, get boss(){return boss;}, get enemies(){return enemies;},
    start(l=1){ startGame(); G.levelIndex=clamp(l-1,0,LEVELS.length-1); loadLevel(G.levelIndex); },
    coop(on){ G.players=on===false?1:2; },
    warpToBoss(){ if(G.state!=='play')this.start(G.levelIndex+1); if(G.lvl.bossTrigger)for(const p of players)p.x=G.lvl.bossTrigger.px-VW*0.5; },
    give(w){ for(const p of players)p.weapon=w; },
